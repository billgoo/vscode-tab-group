...
- Dirty-file decorations are provided by `TabFileDecorationProvider` and applied when building tree items. See [src/decorators/TabFileDecorationProvider.ts](src/decorators/TabFileDecorationProvider.ts#L1-L58).
...
## Security
- The extension operates on VS Code APIs only; no network or filesystem writes are present in the core tree/decoration paths. See [src/providers/TreeDataProvider.ts](src/providers/TreeDataProvider.ts#L17-L245) and [src/decorators/TabFileDecorationProvider.ts](src/decorators/TabFileDecorationProvider.ts#L1-L58).
