...
`test:unit` validates the grouping model without VS Code. `test:e2e` downloads and starts a clean VS Code Extension Development Host, activates the extension, and verifies its public commands are registered. On headless Linux, run it through `xvfb-run -a npm run test:e2e`.
