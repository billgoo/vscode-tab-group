...
`test:unit` validates grouping and saved-group persistence without VS Code. `test:e2e` uses the locally installed VS Code application on macOS, then activates the extension and verifies public commands, Recent Tabs, the default-collapsed expandable Saved Groups panel, saved-tab descriptor capture and text-tab reopening, supported tab-input normalization, and rejection of opaque inputs. Set `VSCODE_TEST_EXECUTABLE` to use a local executable on another platform. In CI it downloads and starts a clean VS Code Extension Development Host. Set `VSCODE_TEST_DOWNLOAD=true` to use the downloaded runtime locally. On headless Linux, run it through `xvfb-run -a npm run test:e2e`.
